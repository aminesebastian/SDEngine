...
EditorSprite::EditorSprite(TString TexturePath, vec3 Color) : EditorSprite(new Texture2D(TexturePath), Color) { }
EditorSprite::EditorSprite(Texture2D* Sprite, vec3 Color) : EditorSprite() {
	this->Sprite = Sprite;
	Tint = Color;
}
EditorSprite::EditorSprite() : Entity("EditorSprite") {
	SpriteVertexArray = 0;
...
void EditorSprite::Draw(Camera* Camera, bool bCallerProvidesShader) {
	if (Sprite == nullptr) {
		return;
	}
	...
	EngineStatics::GetSpriteShader()->Update(GetTransform(), Camera);
...
bool EditorSprite::TraceAgainstRay(vec3 Origin, vec3 Direction, vec3& HitPoint, float& Distance, ECollisionChannel Channel) {
...
void EditorSprite::SetTint(vec3 NewTint) {
	Tint = NewTint;
}
vec3 EditorSprite::GetTint() {
	return Tint;
}