...
void main()	{	
	...
	if(PASS == 0) { 
		...
		vec3 normal		= (transpose(INV_VIEW_MATRIX) * texture(normal, texCoord0)).xyz * 2.0 - 1.0;
		...
		for (int i = 0; i < kernelSize; ++i) {
			vec3 kernelSample	= Samples[i];
			kernelSample		= (kernelSample * radius) + fragPos;
			...
			float sampleDepth = 1-texture(worldPosition, offset.xy).a;

			float rangeCheck= abs(fragPos.z - sampleDepth) < radius ? 1.0 : 0.0;
			occlusion += (sampleDepth <= 1-kernelSample.z + bias ? 1.0 : 0.0) * rangeCheck;
