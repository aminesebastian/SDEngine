...
vec4 Bloom() {
    vec2 tex_offset = BLOOM_SIZE * 1.0 / textureSize(HDR, 0); // gets size of single texel
    vec4 bloom = texture(HDR, texCoord0) * weight[0]; // current fragment's contribution

    for(int i = 1; i < 10; ++i) {
		bloom += texture(HDR, texCoord0 + vec2(0.0, tex_offset.y * i)) * weight[i];
        bloom += texture(HDR, texCoord0 - vec2(0.0, tex_offset.y * i)) * weight[i];
    }
