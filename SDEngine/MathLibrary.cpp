...
bool MathLibrary::LineTraceAgainstWorldFromScreen(FHitInfo& HitInfo, vec2 MouseCoordinates, Camera* Camera, UWorld* World) {
	vec3 origin;
	vec3 direction;
	...
	return LineTraceAgainstWorld(HitInfo, World, origin, direction);
}
bool MathLibrary::LineTraceAgainstWorld(FHitInfo& HitInfo, UWorld* World, vec3 RayOrigin, vec3 RayDirection) {
	...
	for (Entity* temp : World->GetWorldEntities()) {
		if (temp->IsVisible() && temp->HasAxisAlignedBoundingBox()) {
			float dist;
			vec3 collisionPoint;
			bool collide = temp->TraceAgainstRay(RayOrigin, RayDirection, collisionPoint, dist);
	...
	// Check all world lights.
	for (Light* light : World->GetWorldLights()) {
		if (light->IsVisible() && !Engine::GetInstance()->IsInGameMode() && light->HasAxisAlignedBoundingBox()) {
			float dist;
			vec3 collisionPoint;
			bool collide = light->TraceAgainstRay(RayOrigin, RayDirection, collisionPoint, dist);
