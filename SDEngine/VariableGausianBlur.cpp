...
VariableGausianBlur::VariableGausianBlur(vec2 RenderTargetResolution) {
	...
	for (int i = 0; i < DownsampleSteps; i++) {
		float downsampleFactor = glm::sqrt(i + 1);
		DownSampleFactors.push_back(downsampleFactor);

		RenderTarget* firstBuffer = new RenderTarget();
		...
		firstBuffer->Init(RenderTargetResolution.x / downsampleFactor, RenderTargetResolution.y / downsampleFactor);

		RenderTarget* secondBuffer = new RenderTarget();
		...
		secondBuffer->Init(RenderTargetResolution.x / downsampleFactor, RenderTargetResolution.y / downsampleFactor);
