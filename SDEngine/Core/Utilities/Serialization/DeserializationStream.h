...
public:
	...
	void DeserializeCharacterArray(SArray<char>& OutValue);
	void DeserializeUnsignedCharacterArray(SArray<unsigned char>& OutValue);
	...
	void DeserializeCharacter(char& OutValue);
	void DeserializeUnsignedCharacter(unsigned char& OutValue);
private:
	ByteBuffer& Buffer;
	int CurrentIndex;
};
