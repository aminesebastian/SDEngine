...
void DeserializationStream::DeserializeUnsignedCharacterArray(SArray<unsigned char>& OutValue) {
	int32 count = 0;
	DeserializeInteger32(count);

	for (int32 i = 0; i < count; i++) {
		unsigned char temp;
		DeserializeUnsignedCharacter(temp);
		OutValue.Add(temp);
	}
}
...
void DeserializationStream::DeserializeString(TString& Value) {
	...
	CurrentIndex++; // Increment once at the end for the null terminator.
}
...
void DeserializationStream::DeserializeUnsignedCharacter(unsigned char& OutValue) {
	memcpy(&OutValue, &Buffer[CurrentIndex], sizeof(unsigned char));
	CurrentIndex++;
}