...
bool DragFloat::BeginDrawingDragEntries() {
	...
	ImGui::BeginGroup();
	ImGui::PushID(Label.ToCString());
	ImGui::PushMultiItemsWidths(Entries.Count(), ImGui::CalcItemWidth());
	ImGui::TextUnformatted(Label.ToCString(), ImGui::FindRenderedTextEnd(Label.ToCString()));
	return true;
}
bool DragFloat::DrawDragEntry(FDragFloatEntry* Entry, int Index) {
	...
	value_changed = ImGui::DragFloat("##v", &InternalValues[Index], Entry->ValuePerPixel, Entry->Min, Entry->Max, Entry->Format.ToCString(), Entry->Power);
