...
DefferedCompositor::DefferedCompositor(string LightingShader) {
	S_LightingShader = new Shader(LightingShader + ".frag", LightingShader + ".vert");
	S_FinalOutputShader = (new Shader("./Res/Shaders/PostProcessing/Output.frag", "./Res/Shaders/PostProcessing/Output.vert"));
}
...
void DefferedCompositor::CompositePostProcesing(GBuffer* ReadBuffer, GBuffer* WriteBuffer, Camera* Camera, int PostProcessingIndex) {
	...
	for (int i = 0; i < ReadBuffer->GBUFFER_NUM_TEXTURES; i++) {
		...
		glUniform1i(glGetUniformLocation(GetPostProcessShader(PostProcessingIndex)->GetProgram(), ReadBuffer->GetTextureName(i).ToCString()), i);
	}
	DrawScreenQuad();
}
void DefferedCompositor::OutputToScreen(RenderTarget* ReadBuffer) {
	...
	glUniform1i(glGetUniformLocation(S_FinalOutputShader->GetProgram(), ReadBuffer->GetTextureName(0).ToCString()), 0);
