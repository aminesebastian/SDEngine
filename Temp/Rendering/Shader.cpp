...
Shader::Shader(const TString& ShaderName) {
	S_FragmentShaderPath = ShaderName + ".frag";
	S_VertexShaderPath = "./Res/Shaders/DefaultGeometryPassShader.vert";
	int lastSlash = S_FragmentShaderPath.FindLastOf('/');
	S_ShaderName = S_FragmentShaderPath.Substring(lastSlash + 1, S_FragmentShaderPath.Length() - 5);
	RecompileShader();
}
Shader::Shader(const TString& FragmentShaderPath, const TString& VertexShaderPath) {
	S_FragmentShaderPath = FragmentShaderPath;
	S_VertexShaderPath = VertexShaderPath;
	int lastSlash = S_FragmentShaderPath.FindLastOf('/');
	S_ShaderName = S_FragmentShaderPath.Substring(lastSlash + 1, S_FragmentShaderPath.Length() - 5);
	RecompileShader();
}
...
void Shader::RecompileShader() {
	...
	SD_ENGINE_INFO("Shader Compilation Success: {0}", S_ShaderName.ToCString())
}
Shader::~Shader() {
	for (unsigned int i = 0; i < NUM_SHADERS; i++) {
...
GLuint Shader::CreateShader(const TString& text, GLenum ShaderType) {
	...
	shaderSourceStrings[0] = text.ToCString();
...
bool Shader::CheckShaderError(GLuint shader, GLuint flag, bool isProgram, const TString& errorMessage) {
	...
	if (success == GL_FALSE) {
		...
		SD_ENGINE_ERROR("{0}: {1}", errorMessage.ToCString(), error)
	}
	return success;
}
...
void Shader::SetShaderInteger(TString Name, int Value) {
	glUniform1i(glGetUniformLocation(GetProgram(), Name.ToCString()), Value);
}
void Shader::SetShaderVector4(TString Name, vec4 Vector) {
	glUniform4fv(glGetUniformLocation(GetProgram(), Name.ToCString()), 1, &Vector[0]);
}
void Shader::SetShaderVector3(TString Name, vec3 Vector) {
	glUniform3fv(glGetUniformLocation(GetProgram(), Name.ToCString()), 1, &Vector[0]);
}
void Shader::SetShaderVector2(TString Name, vec2 Vector) {
	glUniform2fv(glGetUniformLocation(GetProgram(), Name.ToCString()), 1, &Vector[0]);
}
void Shader::SetShaderFloat(TString Name, float Value) {
	glUniform1f(glGetUniformLocation(GetProgram(), Name.ToCString()), Value);
}
void Shader::SetShaderMatrix3(TString Name, mat3 Matrix) {
	glUniformMatrix3fv(glGetUniformLocation(GetProgram(), Name.ToCString()), 1, GL_FALSE, &Matrix[0][0]);
}
void Shader::SetShaderMatrix4(TString Name, mat4 Matrix) {
	glUniformMatrix4fv(glGetUniformLocation(GetProgram(), Name.ToCString()), 1, GL_FALSE, &Matrix[0][0]);
}
...
TString Shader::GetFramentShaderFilePath() {
	return S_FragmentShaderPath;
}
TString Shader::GetVertexShaderFilePath() {
