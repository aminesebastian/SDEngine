...
bool ByteBuffer::WriteToCompressedFile(TString FilePath) {
	...
	std::ofstream outFile(FilePath.ToCString(), std::ofstream::binary);
	if (!outFile) {
		SD_ENGINE_ERROR("An error occured when attempting to write compressed file: {0}.", FilePath);
		return false;
	}
	outFile.write(compressedData.ToCString(), compressedData.Length());
	outFile.close();
	return true;
}
...
ByteBuffer* ByteBuffer::FromCompressedFile(TString FilePath) {
	std::ifstream file;
	file.open(FilePath.ToCString(), std::ios::binary);
	if (!file) {
